...
const VideoDiscalimer : FunctionComponent<Props> = ({countryCode}) => {
    ...
    const translate = () => translations[countryCode] ? translations[countryCode] : translations['row']

    return (
        <Container>
            {translate()}

            {countryCode === 'it' && <div style={{ display: 'flex', background: 'white', padding: '1rem', justifyContent: 'space-around', alignItems: 'center' }}>
                <LazyImage
                    width={80}
                    height={70}
                    alt='alert icon'
